...
@interface MyCollectionFakeView : UIView
...
@implementation MyCollectionFakeView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ([super initWithFrame:frame]) {
        ...
        self.titleLabel.textColor = HEXColor(0x777777);
...
@interface MyCollectionFlowLayout()<UIGestureRecognizerDelegate>

@property (nonatomic, strong) MyCollectionFakeView *cellFakeView;
...
- (void)autoScroll{
    ...
    //如果刚好拖动到最后一排cell的y轴
    if (CGRectGetMidY(_cellFakeView.frame) >= yoffset) {
...
- (void)handleLongPressGesture:(UILongPressGestureRecognizer *)longPress{
    switch (longPress.state) {
        case UIGestureRecognizerStateBegan: {
            ...
            _cellFakeView = [[MyCollectionFakeView alloc] initWithFrame:cell.frame];
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)pan{
    ...
    switch (pan.state) {
        case UIGestureRecognizerStateChanged: {
            ...
            CGFloat yoffset = 40 + 15 + CGRectGetHeight(self.fakeViewFrame)*(line-1) + 15*(line-1);
            //如果刚好拖动到最后一排cell的y轴位置
            if (CGRectGetMidY(_cellFakeView.frame) >= yoffset) {
                //当到达最后一个cell后在往上拖动时
                if ((yoffset - self.fakeViewOldyoffset) > _panTranslation.y) {
                    _cellFakeView.center = CGPointMake(_cellFakeViewCenter.x + _panTranslation.x, _cellFakeViewCenter.y + _panTranslation.y);
                }else{
                    //不让其越过最后一排cell的y轴位置
